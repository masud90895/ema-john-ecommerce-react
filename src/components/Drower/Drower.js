...
const Drower = ({productItem,setProductItem}) => {
  ...
  // shipping price 
  /* const shippingPrice = productItem.reduce(
    (previousItem, currentItem) =>{
      quantity = quantity + currentItem.quantity;
      const shippingTotal = previousItem + currentItem.shipping;
      return (shippingTotal , quantity);
    },
    0
  ); */
  ...
  // const totalPrice = parseFloat(getItemPrice) + parseFloat(taxPrice) + parseFloat(shippingPrice);
    ...
    return (
        ...
        <div >
          ...
          <h1 className='text-4xl font-bold my-10 text-center'>Oder summery</h1>
            ...
            <h3 className='mb-10 text-xl font-bold'>Grand Total: $<span>{totalPrice}</span></h3>  
           </div>  

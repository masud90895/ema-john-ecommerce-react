...
const Drower = ({productItem,setProductItem}) => {
  ...
  const getItemPrice = productItem.reduce(
    (previousItem, currentItem) => previousItem + currentItem.price,
    0
  );


  const taxPrice = (getItemPrice * 0.1).toFixed(2);

  const totalPrice = parseFloat(getItemPrice) + parseFloat(taxPrice);
  ...
  const clearButton =()=> {
    ...
    return (
        <div className='grid-cols-4 lg:px-5 lg:relative lg:w-[30%]'>
        <div >
          <ul className="menu p-4 overflow-y-auto text-left  w-full lg:w-80  bg-[#ffa72490] text-base-content h-[100vh] lg:fixed">
          <div>
          <h1 className='text-4xl font-bold my-10 text-center'>Oder summery</h1>
          </div>
            <div >
            <h3 className='mb-2'>Selected Items: <span>{productItem.length}</span></h3>
            <h3 className='mb-2'>Total Price: $<span>{getItemPrice}</span></h3>
            <h3 className='mb-2'>Tax: $<span>{taxPrice}</span></h3>
            <h3 className='mb-10'>Grand Total: $<span>{totalPrice}</span></h3>  
           </div>  
